...
/**
 * user-props 노드에 사용자 주요 필드를 분리 저장하는 헬퍼 함수
 *
 * @param {string} uid - 사용자 UID
 * @param {UserData} userData - 사용자 데이터
 * @returns {Record<string, unknown>} user-props 업데이트 객체
 */
function updateUserProps(
  ...
  if (userData.displayName) {
    ...
    // displayName을 소문자로 변환하여 displayNameLowerCase에 저장
    // 대소문자 구분 없는 검색을 위해 사용
    updates[`user-props/displayNameLowerCase/${uid}`] =
 ...
 *
 * 수행 작업:
 * 1. createdAt, updatedAt 필드 자동 생성 (없는 경우)
 * 2. displayName을 소문자로 변환하여 displayNameLowerCase 저장
 * 3. updateUserProps() 함수를 통해 user-props 동기화
 *    - displayName
...
export const onUserCreate = onValueCreated("/users/{uid}", async (event) => {
  ...
  // /users/{uid}에 createdAt, updatedAt 저장 (없는 경우만)
  ...
  if (userData.displayName) {
    updates[`users/${uid}/displayNameLowerCase`] =
      userData.displayName.toLowerCase();
  }

  // updateUserProps() 함수를 통해 user-props 동기화
  const userPropsUpdates = updateUserProps(uid, userData);
  Object.assign(updates, userPropsUpdates);

  // 모든 업데이트를 한 번에 실행
  await admin.database().ref().update(updates);

  logger.info("user-props 동기화 및 createdAt 설정 완료", {
    uid,
    displayName: userData.displayName,
    hasDisplayNameLowerCase: !!userData.displayName,
  });
