 ...
 *
 * 사용 예시:
 * import { addLike, removeLike, checkLikeStatus, listenToLikeStatus } from '$lib/services/like.js';
 *
 * // 좋아요 추가
 ...
 * @param {string} postId - 게시글 ID
 * @param {string} userId - 사용자 UID
 * @param {(isLiked: boolean) => void} callback - 좋아요 상태 변경 시 호출될 콜백
 ...
 * });
 */
export function listenToLikeStatus(postId, userId, callback) {
  try {
    // 좋아요 경로: /post-likes/{postId}-{uid}
    const likeRef = ref(database, `post-likes/${postId}-${userId}`);
 ...
 *
 * @param {string} postId - 게시글 ID
 * @returns {Promise<Array<{uid: string, value: number}>>} 좋아요 목록
 *
 * 기능:
 ...
 * @param {string} postId - 게시글 ID
 * @param {string} userId - 사용자 UID
 * @returns {Promise<{success: boolean, isLiked: boolean, error?: string}>}
 *
 * 기능:
