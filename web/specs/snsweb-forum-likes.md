---
name: snsweb-forum-likes
version: 1.0.0
description: 좋아요 개발 가이드 문서의 SED 사양
author: JaeHo Song
...
## Overview
- 이 문서는 "좋아요 개발 가이드"에 대한 세부 사양을 정리하며, 기존 내용을 그대로 유지한 채 SED 구조에 맞춰 제공합니다.
...
좋아요 기능을 구현할 때, **클라이언트와 백엔드(Cloud Functions)의 역할을 명확히 구분**해야 합니다.

**원칙:**
- ✅ **클라이언트는 최소한의 데이터만 RTDB에 기록**
  - 좋아요 기록만 저장 (`/likes/post-{postId}-{uid}`)
  - 카운터 필드(`likeCount`)는 직접 수정하지 않음
- ✅ **백엔드(Cloud Functions)는 카운터 자동 처리**
  - 좋아요 추가/삭제 시 `likeCount` 자동 증가/감소
  - 전체 통계 자동 집계 (`stats/counters/like`)
  - 데이터 무결성 보장

**클라이언트에서 하지 말아야 할 작업:**
- ❌ `likeCount` 직접 증가/감소 (`increment()` 사용 금지)
- ❌ 전체 좋아요 개수 직접 계산 및 저장
- ❌ 복잡한 비즈니스 로직 처리

**예시:**

```javascript
// ❌ 잘못된 방법: 클라이언트에서 카운터 직접 증가
const updates = {};
updates['likes/post-abc123-user456'] = 1;
updates['posts/abc123/likeCount'] = increment(1);  // ❌ 금지!
await update(ref(database), updates);

// ✅ 올바른 방법: 클라이언트는 최소 데이터만 저장
const updates = {};
updates['likes/post-abc123-user456'] = 1;  // 좋아요 기록만
await update(ref(database), updates);
// → Cloud Functions가 자동으로 likeCount를 증가시킵니다
```

**이점:**
- 🔒 **데이터 무결성**: 이중 증가/감소 방지, 일관된 데이터 처리
- ⚡ **성능**: 클라이언트는 단순 작업만 수행
- 🔧 **유지보수성**: 비즈니스 로직이 백엔드에 집중
- 🌐 **플랫폼 독립성**: 웹/앱 모두 동일한 백엔드 로직 공유

자세한 Cloud Functions 구현은 아래 [Cloud Functions 연동](#cloud-functions-연동) 섹션을 참고하세요.
